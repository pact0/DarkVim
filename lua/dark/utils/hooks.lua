...
function M.run_on_packer_complete()
  ...
  pcall(vim.cmd, "colorscheme " .. dark.colorscheme)
...
function M.reset_cache()
  ...
  for module, _ in pairs(package.loaded) do
    if module:match "dark.core" or module:match "dark.lsp" then
      package.loaded[module] = nil
      table.insert(dark_modules, module)
    end
  end
  Log:trace(string.format("Cache invalidated for core modules: { %s }", table.concat(dark_modules, ", ")))
...
function M.run_post_update()
  ...
  if vim.fn.has "nvim-0.7" ~= 1 then
    ...
    vim.notify(
      "Please upgrade your Neovim base installation. Newer version of Darkvim requires v0.7+",
      vim.log.levels.WARN
    )
    vim.wait(1000, function()
      return false
    end)
    local ret = require_clean("dark.utils.git").switch_dark_branch(compat_tag)
    if ret then
