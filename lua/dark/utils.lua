...
local function r_inspect_settings(structure, limit, separator)
  ...
  if type(structure) == "function" then
    ...
    io.write("-- dark", separator:sub(2), " = function ()\n")
  else
    io.write("dark", separator:sub(2), " = ", tostring(structure), "\n")
  end
  return limit - 1
end
...
function M.generate_settings()
  ...
  -- write all `dark` related settings to `lv-settings.lua` file
  r_inspect_settings(dark, 10000, ".")
