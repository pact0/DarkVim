...
local default_snapshot = join_paths(get_dark_base_dir(), "snapshots", "default.json")
...
function plugin_loader.init(opts)
  ...
  if status_ok then
    packer.on_complete = vim.schedule_wrap(function()
      require("dark.utils.hooks").run_on_packer_complete()
    end)
...
function plugin_loader.load(configurations)
  ...
  local status_ok, _ = xpcall(function()
    ...
    vim.cmd("colorscheme " .. dark.colorscheme)
  end, debug.traceback)
  if not status_ok then
...
function plugin_loader.load_snapshot(snapshot_file)
  ...
  if not in_headless then
    vim.notify("Syncing core plugins is in progress..", vim.log.levels.INFO, { title = "dark" })
  end
...
function plugin_loader.sync_core_plugins()
  ...
  vim.api.nvim_create_autocmd("User", {
    ...
    once = true,
    callback = function()
      require("dark.plugin-loader").load_snapshot(default_snapshot)
    end,
  })
