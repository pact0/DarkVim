...
function Log:set_level(level)
  local logger_ok, _ = xpcall(function()
    ...
    if structlog then
      local logger = structlog.get_logger "dark"
  ...
  local packer_ok, _ = xpcall(function()
    ...
    require("packer.log").new { level = dark.log.level }
  end, debug.traceback)
...
function Log:init()
  ...
  local log_level = Log.levels[(dark.log.level):upper() or "WARN"]
  local dark_log = {
    dark = {
      sinks = {
  ...
  structlog.configure(dark_log)
  local logger = structlog.get_logger "dark"

  -- Overwrite `vim.notify` to use the logger
  if dark.log.override_notify then
...
function Log:get_path()
  return string.format("%s/%s.log", get_cache_dir(), "dark")
end
