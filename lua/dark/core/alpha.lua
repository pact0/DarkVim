...
function M.config()
  local dark_dashboard = require "dark.core.alpha.dashboard"
  local dark_startify = require "dark.core.alpha.startify"
  dark.builtin.alpha = {
    dashboard = { config = {}, section = dark_dashboard.get_sections() },
    startify = { config = {}, section = dark_startify.get_sections() },
    active = true,
    mode = "dashboard",
  }
end
...
local function resolve_config(theme_name)
  ...
  local section = dark.builtin.alpha[theme_name].section
...
local function configure_additional_autocmds()
  ...
  if not dark.builtin.lualine.options.globalstatus then
...
function M.setup()
  ...
  local mode = dark.builtin.alpha.mode
  local config = dark.builtin.alpha[mode].config
