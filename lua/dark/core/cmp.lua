  ...
  dark.builtin.cmp = {
    ...
    formatting = {
      ...
      format = function(entry, vim_item)
        local max_width = dark.builtin.cmp.formatting.max_width
        if max_width ~= 0 and #vim_item.abbr > max_width then
          vim_item.abbr = string.sub(vim_item.abbr, 1, max_width - 1) .. "…"
        end
        if dark.use_icons then
          vim_item.kind = dark.builtin.cmp.formatting.kind_icons[vim_item.kind]
        end
    ...
    mapping = cmp.mapping.preset.insert {
      ...
      ["<CR>"] = cmp.mapping(function(fallback)
        if cmp.visible() and cmp.confirm(dark.builtin.cmp.confirm_opts) then
          if jumpable() then
            luasnip.jump(1)
          end
          return
...
function M.setup()
  require("cmp").setup(dark.builtin.cmp)
end
