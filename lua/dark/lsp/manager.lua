...
local function resolve_config(server_name, ...)
  local defaults = {
    on_attach = require("dark.lsp").common_on_attach,
    on_init = require("dark.lsp").common_on_init,
    on_exit = require("dark.lsp").common_on_exit,
    capabilities = require("dark.lsp").common_capabilities(),
  }

  local has_custom_provider, custom_config = pcall(require, "dark/lsp/providers/" .. server_name)
...
function M.setup(server_name, user_config)
  ...
  if dark_lsp_utils.is_client_active(server_name) or client_is_configured(server_name) then
    return
  end
  ...
  if not server:is_installed() then
    if dark.lsp.automatic_servers_installation then
