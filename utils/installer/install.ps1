...
function main($cliargs) {
    ...
    if ($cliargs.Contains("--overwrite")) {
        Write-Output "!!Warning!! -> Removing all darkvim related config because of the --overwrite flag"
    ...
    if ($cliargs.Contains("--local") -or $cliargs.Contains("--testing")) {
        msg "Using local DarkVim installation"
        local_install
        exit
    }
    ...
    $answer = Read-Host "Would you like to check darkvim's NodeJS dependencies? [y]es or [n]o (default: no) "
    if ("$answer" -eq "y" -or "$answer" -eq "Y") {
        install_nodejs_deps
    }

    $answer = Read-Host "Would you like to check darkvim's Python dependencies? [y]es or [n]o (default: no) "
    if ("$answer" -eq "y" -or "$answer" -eq "Y") {
        install_python_deps
    }


    if (Test-Path "$env:DARKVIM_BASE_DIR\init.lua" ) {
        msg "Updating DarkVim"
...
function local_install() {
    ...
    $gitLocalCloneCmd = git clone --progress "$repoDir" "$env:DARKVIM_BASE_DIR"
    Invoke-Command -ErrorAction Stop -ScriptBlock { $gitLocalCloneCmd; setup_dark }
}
...
function setup_shim() {
    ...
    Copy-Item -Force "$env:DARKVIM_BASE_DIR\utils\bin\dark.ps1" "$INSTALL_PREFIX\bin\dark.ps1"
}

function uninstall_dark() {
...
function setup_dark() {
    ...
    if (Test-Path "$env:DARKVIM_CONFIG_DIR\config.lua") {
        Move-Item "$env:DARKVIM_CONFIG_DIR\config.lua" "$env:DARKVIM_CONFIG_DIR\config.lua.old"
    }

    New-Item -ItemType File -Path "$env:DARKVIM_CONFIG_DIR\config.lua" | Out-Null

    $exampleConfig = "$env:DARKVIM_BASE_DIR\utils\installer\config_win.example.lua"
    Copy-Item -Force "$exampleConfig" "$env:DARKVIM_CONFIG_DIR\config.lua"

    # FIXME: this has never worked
    # Invoke-Expression "$INSTALL_PREFIX\bin\dark.ps1 --headless -c 'autocmd User PackerComplete quitall' -c 'PackerSync'"
    ...
    Write-Output "You can start it by running: $INSTALL_PREFIX\bin\dark.ps1"
...
function validate_darkvim_files() {
    Set-Alias dark "$INSTALL_PREFIX\bin\dark.ps1"
    try {
        $verify_version_cmd='if v:errmsg != "" | cquit | else | quit | endif'
        Invoke-Command -ScriptBlock { dark --headless -c 'DarkUpdate' -c "$verify_version_cmd" } -ErrorAction SilentlyContinue
    }
    catch {
        Write-Output "Unable to guarantee data integrity while updating. Please run `:DarkUpdate` manually instead."
        exit 1
    }
    Write-Output "Your DarkVim installation is now up to date!"
}
...
function create_alias {
    try {
        $answer = Read-Host $(`
                ...
                "(This enables you to start dark with the command 'dark') [y]es or [n]o (default: no)" )
    }
    catch {
    ...
    $dark_bin="$INSTALL_PREFIX\bin\dark.ps1"
    $dark_alias = Get-Alias dark -ErrorAction SilentlyContinue

    if ($dark_alias.Definition -eq $dark_bin) {
        Write-Output "Alias is already set and will not be reset."
        return
    }

    Add-Content -Path $PROFILE -Value $("`r`nSet-Alias dark $dark_bin")
